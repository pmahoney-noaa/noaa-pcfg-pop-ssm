...
//  - Lambda [t+1] is conditioned on mean Lambda and Lambda[t]
...
generated quantities{  // ------------------------------------------------------
 ...
 logLambda_proj[1] = normal_rng(mu_logLambda + logLambda[n_dat_yrs] * beta, sigma_logLambda); // first project year Lambda
 logN_proj[1] = log(exp(normal_rng(logN[n_dat_yrs] + (mu_logLambda + logLambda_proj[1] * beta), sigma_logLambda)) - N_harvest[1]);  
 
 for(t_proj in 2:n_proj_yrs){
   logLambda_proj[t_proj] = normal_rng(mu_logLambda + logLambda_proj[t_proj - 1] * beta, sigma_logLambda);
   logN_proj[t_proj] = log(exp(normal_rng(logN_proj[t_proj - 1] + (mu_logLambda + logLambda_proj[t_proj] * beta), sigma_logLambda)) - N_harvest[t_proj]);
 }
 
 // project abundance in log-space for data year t; used in determing model performance
 for (t in 1:(n_dat_yrs - 1)){
   logLambda_pred[1] = normal_rng(mu_logLambda + logLambda[t] * beta, sigma_logLambda); 
   logN_pyear1[t] = normal_rng(logN[t] + (mu_logLambda + logLambda_pred[1] * beta), sigma_logLambda);
   
   logLambda_pred[2] = normal_rng(mu_logLambda + logLambda_pred[1] * beta, sigma_logLambda);
   logN_pyear2[t] = normal_rng(logN_pyear1[t] + (mu_logLambda + logLambda_pred[2] * beta), sigma_logLambda);
   
   logLambda_pred[3] = normal_rng(mu_logLambda + logLambda_pred[2] * beta, sigma_logLambda);
   logN_pyear3[t] = normal_rng(logN_pyear2[t] + (mu_logLambda + logLambda_pred[3] * beta), sigma_logLambda);
 }
}
