...
//  - Lambda [t+1] is conditioned on Lambda[t]; Lambda is now modeled as the difference between
//    years to facilitate model fitting.
...
model {  // --------------------------------------------------------------------
  ...
  logLambda[1] ~ normal(0, sigma_logLambda);
  for (t in 1:(n_dat_yrs - 1)){
   deltaLambda[t] ~ normal(0, sigma_logLambda); 
  }
