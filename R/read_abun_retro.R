...
# Read Data --------------------------------------------------------------------
retro_dat_raw = read_excel(
  path = here("data", "PCFG_retrospective_abundance_time_series.xlsx"),
  sheet = "N_retro_long"
) # sheet = "All time series together"
...
retro_dat = retro_dat_raw %>%
  ...
  ungroup() %>%
  mutate(
    N_delta_perc = ifelse(!is.na(N), (N - init_N) / init_N, NA),
...
plot_abund_paired = rbind(Ndata_last, Ndata) %>%
  ...
  geom_errorbar(
    aes(ymin = low_95CI, ymax = high_95CI, width = 0.25),
