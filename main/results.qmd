...
source(here("R", "read_calf_pcfg_retro.R")) #; N_pcfg_calves; tail(N_pcfg_calves)  # Check
...
# !! TEMPORARY DATA FILL TO FACILITATE PRELIM MODEL FITTING !!
Cdata_input <- Cdata_input %>%
  add_row(Cdata_input %>% filter(year == max(year))) # Has no impact on model outcomes, needed for STAN

N_pcfg_calves <- N_pcfg_calves %>%
  add_row(year = 2025, N = 8) %>%
  add_row(year = 2026, N = 0)       # Has no impact on model outcomes, needed for STAN

Sdata <- Sdata %>%
  add_row(species = "Whale, gray", state = "All", year = 2024, N_strand = 60) %>%
  add_row(species = "Whale, gray", state = "All", year = 2025, N_strand = 85) %>%
  add_row(species = "Whale, gray", state = "All", year = 2026, N_strand = 15)
...
x_lambda = data.frame(
  N_pcfg_calves = N_pcfg_calves %>% pull(N),
  N_strandings = Sdata %>% filter(state == "All" & year >= 2002) %>% pull(N_strand)
)
