...
knitr::kable(
  ...
  col.names = c("Model", "Mean RSS", #"Median RSS", 
                ...
                "Number of Closures (Nmin)", "Number of Closures (N)",
                "Number of Closures", "Closure Years"),
  digits = c(rep(1, 3), rep(3, 4), rep(0, 4)),
  align = c("l", rep("c", 8)),
  format = "simple",
  booktabs = T
) %>%
...
#     model = "Model",
#     mnRSS = "Mean RSS",
#     mnPercentile_abundEstN = "Mean Percentile (N)",
#     mdPercentile_abundEstN = "Median Percentile (N)",
#     mnProp_below_threshold = "Mean Prop Below Threshold (N)",
#     mdProp_below_threshold = "Median Prop Below Threshold (N)",
#     Nclosures = "Number of Closures",
#     closure_years = "Closure Years"
#   ) %>%
#   tab_options(
...
```{r fig-trendFig, echo = F, warning = F, message = F, fig.height = 18, fig.width = 8, dpi = 300}
#| label: fig-trendFig
...
```{r modelWeights, echo = F, warning = F, message = F}
#| label: tbl-modelWeights
#| tbl-cap: "Model weights..."

# Model weights
wgts_stacked <- loo_model_weights(loo_pcfg)
wgts <- loo_model_weights(loo_pcfg, method = "pseudobma", BB = TRUE)

wgts_tab <- data.frame(Model = model_names, Stacking = as.numeric(wgts_stacked), PseudoBMA = as.numeric(wgts))
...
```{r fig-stackedTrendFig, echo = F, warning = F, message = F, fig.height = 4, fig.width = 8, dpi = 300}
...
ma_abund <- tidy_model_avg(tfit, wgts, iters = 1000, Ndata_input)
...
#| tbl-cap: "Model-predicted N and N~min~ in 2025 based on a two year projection from the last abundance estimate (2023)."
...
# Pulling the model-averaged estimate
ma_est <- ma_abund %>%
  slice(n()) %>%
  mutate(
    Model = "Model-averaged estimate"
  ) %>%
  dplyr::select(Model, Nmean = mean, Nmin = percentile_20)
...
#| fig-cap: "Model-specific estimated abundance in the second year of projections (2025). Mean estimates are represented by vertical dashed lines in the corresponding colors. The black and red ticks along the x-axis (rugs) correspond to the management closure thresholds for N and N minimum, respectively."
